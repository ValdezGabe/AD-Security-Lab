...
## Before adding new screenshots

Check every capture for:

- **Windows product keys** — not the Product ID (an installation identifier, harmless), but an actual 5x5 activation key
- **Passwords or credentials** in terminal scrollback
- **Student IDs, employee IDs**, or other personal identifiers
- **Real hostnames or public IPs** — RFC1918 lab addresses (`172.16.x.x`, `192.168.x.x`) are fine
- **Collaborator names** in document headers or window titles

Naming convention: `<doc-number>-<subject>.png`
